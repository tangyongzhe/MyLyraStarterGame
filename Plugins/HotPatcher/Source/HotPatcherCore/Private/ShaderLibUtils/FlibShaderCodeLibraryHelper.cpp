...
void UFlibShaderCodeLibraryHelper::CancelMaterialShaderCompile(UMaterialInterface* MaterialInterface)
{
	if(MaterialInterface)
	{
		...
		{
			if (const FMaterialResource* Res = MaterialInterface->GetMaterialResource((ERHIFeatureLevel::Type)FeatureLevel))
			{
				const_cast<FMaterialResource*>(Res)->CancelCompilation();
			}
		}
	}
}
