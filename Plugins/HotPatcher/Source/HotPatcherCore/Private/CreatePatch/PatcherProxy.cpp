			...
			for(auto& Chunk:Context.PakChunks)
			{
				...
				if(Context.GetSettingObject()->IsCookPatchAssets() || Context.GetSettingObject()->GetIoStoreSettings().bIoStore)
				{
					...
					if(Context.GetSettingObject()->IsCookPatchAssets() || bSharedShaderLibrary)
					{
						...
						for (const TPair<TObjectPtr<UClass>, int32>& OverrideEntry : Context.GetSettingObject()->CookAdvancedOptions.GetOverrideNumberOfAssetsPerFrame())
						{
							EmptySetting.OverrideNumberOfAssetsPerFrame.Add(OverrideEntry.Key.Get(), OverrideEntry.Value);
						}
