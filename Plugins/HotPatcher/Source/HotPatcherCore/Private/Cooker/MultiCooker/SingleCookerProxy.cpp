...
void USingleCookerProxy::PreGeneratePlatformData(const FCookCluster& CookCluster,bool bWaitFinished)
{
	...
	auto PreCachePlatformDataForPackages = [&](TArray<UPackage*>& PreCachePackages)
	{
		...
		for(int32 Index = PreCachePackages.Num() - 1 ;Index >= 0;--Index)
		{
			...
			TSet<UObject*> ProcessedObjectsRaw = ToRawObjectSet(CookerPreCacheDDC.ProcessedObjects);
			TSet<UObject*> PendingCachePlatformDataObjectsRaw = ToRawObjectSet(CookerPreCacheDDC.PendingCachePlatformDataObjects);
			UFlibHotPatcherCoreHelper::CacheForCookedPlatformData(
				TArray<UPackage*>{CurrentPackage},
				TargetPlatforms,
				ProcessedObjectsRaw,
				PendingCachePlatformDataObjectsRaw,
				bConcurrentSave,
			...
			SyncObjectSetFromRaw(ProcessedObjectsRaw, CookerPreCacheDDC.ProcessedObjects);
			SyncObjectSetFromRaw(PendingCachePlatformDataObjectsRaw, CookerPreCacheDDC.PendingCachePlatformDataObjects);
	...
	{
		TArray<UPackage*> PreCachePackages = ToRawPackageArray(CookerPreCacheDDC.Packages);
	...
	if(bWaitFinished)
	{
		TSet<UObject*> ProcessedObjectsRaw = ToRawObjectSet(CookerPreCacheDDC.ProcessedObjects);
		TSet<UObject*> PendingCachePlatformDataObjectsRaw = ToRawObjectSet(CookerPreCacheDDC.PendingCachePlatformDataObjects);
		UFlibHotPatcherCoreHelper::WaitObjectsCachePlatformDataComplete(ProcessedObjectsRaw,PendingCachePlatformDataObjectsRaw,TargetPlatforms);
		SyncObjectSetFromRaw(ProcessedObjectsRaw, CookerPreCacheDDC.ProcessedObjects);
		SyncObjectSetFromRaw(PendingCachePlatformDataObjectsRaw, CookerPreCacheDDC.PendingCachePlatformDataObjects);
	}
}
