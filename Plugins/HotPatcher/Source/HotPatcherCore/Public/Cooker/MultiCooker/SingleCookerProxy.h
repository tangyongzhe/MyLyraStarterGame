...
struct FCookerPreCacheDDC
{
    ...
    UPROPERTY()
    TSet<TObjectPtr<UObject>> ProcessedObjects;
    UPROPERTY()
    TSet<TObjectPtr<UObject>> PendingCachePlatformDataObjects;
...
inline TArray<UPackage*> ToRawPackageArray(const TArray<TObjectPtr<UPackage>>& Packages)
{
	TArray<UPackage*> Result;
	Result.Reserve(Packages.Num());
	for (const TObjectPtr<UPackage>& Package : Packages)
	{
		Result.Add(Package.Get());
	}
	return Result;
}

inline TSet<UObject*> ToRawObjectSet(const TSet<TObjectPtr<UObject>>& Objects)
{
	TSet<UObject*> Result;
...
inline void SyncObjectSetFromRaw(const TSet<UObject*>& RawObjects, TSet<TObjectPtr<UObject>>& Objects)
{
	Objects.Empty();
	for (UObject* Object : RawObjects)
	{
		Objects.Add(Object);
	}
}
