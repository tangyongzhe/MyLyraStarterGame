...
void FJsEnvImpl::LoadUEType(const v8::FunctionCallbackInfo<v8::Value>& Info)
{
    ...
    if (!Type && !TypeName.StartsWith(TEXT("/")))
    {
        const FString LyraGameScriptPath = FString::Printf(TEXT("/Script/LyraGame.%s"), *TypeName);
        Type = LoadObject<UClass>(nullptr, *LyraGameScriptPath);

        if (!Type)
        {
            Type = LoadObject<UScriptStruct>(nullptr, *LyraGameScriptPath);
        }

        if (!Type)
        {
            Type = LoadObject<UEnum>(nullptr, *LyraGameScriptPath);
        }
    }
