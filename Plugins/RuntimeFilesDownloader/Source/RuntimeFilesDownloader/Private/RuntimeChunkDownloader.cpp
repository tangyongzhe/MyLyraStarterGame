...
TFuture<FRuntimeChunkDownloaderResult> FRuntimeChunkDownloader::DownloadFileByChunk(const FString& URL, float Timeout, const FString& ContentType, int64 ContentSize, FInt64Vector2 ChunkRange, const TFunction<void(int64, int64)>& OnProgress)
{
	...
	HttpRequestRef->OnRequestProgress64().BindLambda([WeakThisPtr, ContentSize, ChunkRange, OnProgress](FHttpRequestPtr Request, uint64 BytesSent, uint64 BytesReceived)
	{
		TSharedPtr<FRuntimeChunkDownloader> SharedThis = WeakThisPtr.Pin();
		if (SharedThis.IsValid())
		{
			const float Progress = ContentSize <= 0 ? 0.0f : static_cast<float>(BytesReceived) / ContentSize;
			UE_LOG(LogRuntimeFilesDownloader, Log, TEXT("Downloaded %d bytes of file chunk from %s. Range: {%lld; %lld}, Overall: %lld, Progress: %f"), static_cast<int32>(BytesReceived), *Request->GetURL(), ChunkRange.X, ChunkRange.Y, ContentSize, Progress);
			OnProgress(static_cast<int64>(BytesReceived), ContentSize);
		}
	});
...
TFuture<FRuntimeChunkDownloaderResult> FRuntimeChunkDownloader::DownloadFileByPayload(const FString& URL, float Timeout, const FString& ContentType, const TFunction<void(int64, int64)>& OnProgress)
{
	...
	HttpRequestRef->OnRequestProgress64().BindLambda([WeakThisPtr, OnProgress](FHttpRequestPtr Request, uint64 BytesSent, uint64 BytesReceived)
	{
		TSharedPtr<FRuntimeChunkDownloader> SharedThis = WeakThisPtr.Pin();
		if (SharedThis.IsValid())
		{
			...
			UE_LOG(LogRuntimeFilesDownloader, Log, TEXT("Downloaded %d bytes of file chunk from %s by payload. Overall: %lld, Progress: %f"), static_cast<int32>(BytesReceived), *Request->GetURL(), static_cast<int64>(Request->GetContentLength()), Progress);
			OnProgress(static_cast<int64>(BytesReceived), ContentLength);
		}
	});
